#!/usr/bin/env bash
# Measure attribute superposition in code space for one SM-ELF checkpoint.
# Usage: run_eval_superposition.sh <checkpoint_path> [cuda_device] [config] [extra args...]
set -euo pipefail
cd "$(dirname "$0")/.."
CKPT="${1:?usage: run_eval_superposition.sh <checkpoint_path> [cuda_device] [config] [extra...]}"
CUDA_DEV="${2:-7}"
...
exec python3 src/eval_superposition.py --config "$CONFIG" --checkpoint_path "$CKPT" --label-stories 600 "$@"
